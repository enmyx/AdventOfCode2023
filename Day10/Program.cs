...
namespace Day10
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int y = 1; y < map.GetLength(0); y++)
            {
                for (int x = 1; x < map.GetLength(1); x++)
                {
                    ...
                    walls = 0;
                    // RIGHT
                    for (int i = x + 1; i < map.GetLength(1); i++)
                    {
        ...
        private static void Part1((int y, int x) current, Direction d, char[,] pipes, char[,] map)
        {
            int steps = 0;
            do
            {
                map[current.y, current.x] = pipes[current.y, current.x];
                switch (d)
                {
                    case Direction.Up:
                        current.y--;
                ...
                switch (pipes[current.y, current.x])
                {
                    case 'F':
                        d = d == Direction.Up ? Direction.Right : Direction.Down;
                        break;
                    case '7':
                        d = d == Direction.Up ? Direction.Left : Direction.Down;
                        break;
                    case 'J':
                        d = d == Direction.Down ? Direction.Left : Direction.Up;
                        break;
                    case 'L':
                        d = d == Direction.Down ? Direction.Right : Direction.Up;
                        break;
                    case '|':
                        d = d == Direction.Down ? Direction.Down : Direction.Up;
                        break;
                    case '-':
                        d = d == Direction.Left ? Direction.Left : Direction.Right;
                        break;
                }

                steps++;
            } while (pipes[current.y, current.x] != 'S');
